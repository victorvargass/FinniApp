...
async function initializeDatabase(): Promise<void> {
  ...
  await db.execAsync(`
    ...
    CREATE TABLE IF NOT EXISTS savings_goals (
      ...
      color TEXT NOT NULL DEFAULT '#0a7ea4',
      group_id INTEGER REFERENCES savings_groups(id) ON DELETE SET NULL,
  ...
  if (!incomeColumns.some((column) => column.name === 'category_id')) {
    await db.execAsync('ALTER TABLE incomes ADD COLUMN category_id INTEGER REFERENCES income_categories(id) ON DELETE SET NULL;');
  }
  ...
  if (!savingsGoalColumns.some((column) => column.name === 'group_id')) {
    await db.execAsync('ALTER TABLE savings_goals ADD COLUMN group_id INTEGER REFERENCES savings_groups(id) ON DELETE SET NULL;');
  }
  ...
  if (!savingsAdjustmentColumns.some((column) => column.name === 'reported_balance')) {
    await db.execAsync('ALTER TABLE savings_goal_adjustments ADD COLUMN reported_balance INTEGER;');
  }
  ...
  const manualDebtColumns = await db.getAllAsync<{ name: string }>(
    'PRAGMA table_info(manual_debts)'
  );
  if (!manualDebtColumns.some((column) => column.name === 'balance_updated_at')) {
    await db.execAsync('ALTER TABLE manual_debts ADD COLUMN balance_updated_at TEXT;');
  }
  if (!manualDebtColumns.some((column) => column.name === 'balance_payment_anchor_id')) {
    await db.execAsync('ALTER TABLE manual_debts ADD COLUMN balance_payment_anchor_id INTEGER NOT NULL DEFAULT 0;');
  }
  if (!manualDebtEntryColumns.some((column) => column.name === 'reported_balance')) {
    await db.execAsync('ALTER TABLE manual_debt_entries ADD COLUMN reported_balance INTEGER;');
  }
  ...
  if (previousSchemaVersion < 14) {
    await db.runAsync(`
      ...
      WHERE balance_updated_at IS NULL
        ...
        AND (
          NOT EXISTS (
            SELECT 1 FROM manual_debt_entries payment
            WHERE payment.debt_id = manual_debts.id AND payment.kind = 'payment'
          )
          OR COALESCE((
            SELECT MAX(payment.date) FROM manual_debt_entries payment
            WHERE payment.debt_id = manual_debts.id AND payment.kind = 'payment'
          ), '9999-12-31') < date(updated_at, 'localtime')
        )
    `);
  }
  if (previousSchemaVersion < 17) {
    // Older builds used the historical creation date for the balance declared
    // at registration. Only the last local update date remains recoverable.
    await db.runAsync(LEGACY_SAVINGS_BALANCE_DATE_SQL);
    await db.runAsync(LEGACY_DEBT_BALANCE_DATE_SQL);
  }
  ...
  if (!recurringIncomeColumns.some((column) => column.name === 'category_id')) {
    await db.execAsync('ALTER TABLE recurring_incomes ADD COLUMN category_id INTEGER REFERENCES income_categories(id) ON DELETE SET NULL;');
  }
...
export async function getIncomeCategories(): Promise<IncomeCategory[]> {
  const db = await getDb();
  return db.getAllAsync<IncomeCategory>('SELECT id, name, color FROM income_categories ORDER BY name COLLATE NOCASE');
}
...
export async function saveIncomeCategory(data: NewIncomeCategory, id?: number): Promise<void> {
  ...
  const duplicate = await db.getFirstAsync<{ id: number }>(
    'SELECT id FROM income_categories WHERE name = ? COLLATE NOCASE AND id != ? LIMIT 1', name, id ?? -1
  );
  if (duplicate) throw new Error(t('database.categoryNameExists'));
  if (id == null) {
    await db.runAsync('INSERT INTO income_categories (name, color) VALUES (?, ?)', name, color);
  } else {
    await db.runAsync('UPDATE income_categories SET name = ?, color = ? WHERE id = ?', name, color, id);
  }
}
...
async function getSavingsGoalBalance(
  ...
  const row = await db.getFirstAsync<{
    ...
    `SELECT
       COALESCE(snapshot.reported_balance, g.initial_amount) + COALESCE(SUM(CASE
         ...
         ELSE 0
       END), 0)
       + CASE WHEN snapshot.id IS NULL THEN COALESCE((
     ...
     SELECT
       ...
       + COALESCE(SUM(CASE
         ...
         WHEN EXISTS (SELECT 1 FROM snapshot)
           AND COALESCE(expense.date, income.date) < (SELECT date FROM snapshot) THEN 0
         WHEN EXISTS (SELECT 1 FROM snapshot)
         ...
         ELSE 0
       END), 0)
       + CASE WHEN NOT EXISTS (SELECT 1 FROM snapshot) THEN COALESCE((
         SELECT SUM(adjustment.amount)
         FROM savings_goal_adjustments adjustment
...
export async function getSavingsGroups(): Promise<SavingsGroup[]> {
  const db = await getDb();
  return db.getAllAsync<SavingsGroup>('SELECT id, name, color FROM savings_groups ORDER BY name COLLATE NOCASE');
}

export async function saveSavingsGroup(data: NewSavingsGroup, id?: number): Promise<void> {
  const db = await getDb();
  const name = data.name.trim();
  const color = data.color.toLowerCase();
  if (!name) throw new Error(t('categories.missingName'));
  const duplicate = await db.getFirstAsync<{ id: number }>(
    'SELECT id FROM savings_groups WHERE name = ? COLLATE NOCASE AND id != ? LIMIT 1', name, id ?? -1
  );
  if (duplicate) throw new Error(t('groupings.savingsGroupNameExists'));
  if (id == null) {
    await db.runAsync('INSERT INTO savings_groups (name, color) VALUES (?, ?)', name, color);
  } else {
    await db.runAsync('UPDATE savings_groups SET name = ?, color = ? WHERE id = ?', name, color, id);
  }
}

export async function deleteSavingsGroup(id: number): Promise<void> {
  const db = await getDb();
  await withExclusiveTransaction(db, async (transaction) => {
    await transaction.runAsync('UPDATE savings_goals SET group_id = NULL WHERE group_id = ?', id);
    await transaction.runAsync('DELETE FROM savings_groups WHERE id = ?', id);
  });
}
...
export async function addSavingsGoalBalanceAdjustment(
  ...
  if (!isValidIsoDate(data.date) || data.date > toLocalIsoDate(new Date())) {
    throw new Error(t('database.savingsAdjustmentDateInvalid'));
  }
  const db = await getDb();
  await withExclusiveTransaction(db, async (transaction) => {
    ...
    const baseline = await transaction.getFirstAsync<{ balance_updated_at: string | null }>(
      'SELECT balance_updated_at FROM savings_goals WHERE id = ?', goalId
    );
    if (baseline?.balance_updated_at && data.date < baseline.balance_updated_at) {
...
export async function updateSavingsGoal(id: number, data: NewSavingsGoal): Promise<void> {
  ...
  await withExclusiveTransaction(db, async (transaction) => {
    ...
    const latestMovement = existing.balance_updated_at === data.balanceDate
      ? null
    ...
    const anchor = existing.balance_updated_at === data.balanceDate
      ? existing.balance_movement_anchor_id
...
export async function getCardPaymentMovementsForPeriod(
  periodId: number
): Promise<CardPaymentMovement[]> {
  const db = await getDb();
  const rows = await db.getAllAsync<Record<string, unknown>>(
    `SELECT * FROM (
       SELECT
       ...
       WHERE expense.period_id = ? AND expense.credit_payment_target_id IS NOT NULL
  ...
  return rows.map((row) => ({
    id: Number(row.id),
    kind: row.kind as CardPaymentMovement['kind'],
    ...
    sourcePaymentMethodName: row.source_payment_method_name == null
      ? null
      : String(row.source_payment_method_name),
    sourcePaymentMethodColor: row.source_payment_method_color == null
      ? null
      : String(row.source_payment_method_color),
    targetPaymentMethodId: Number(row.target_payment_method_id),
    targetPaymentMethodName: String(row.target_payment_method_name),
    targetPaymentMethodColor: String(row.target_payment_method_color),
...
function validatePaymentMethod(data: NewPaymentMethod) {
  ...
  if (data.type !== 'cash' && data.reportedBalance != null
    && (!data.balanceDate || !isValidIsoDate(data.balanceDate)
      || data.balanceDate > toLocalIsoDate(new Date()))) {
...
export async function updateDebt(id: number, data: NewDebt): Promise<void> {
  ...
  await withExclusiveTransaction(db, async (transaction) => {
    ...
    const firstSnapshot = await transaction.getFirstAsync<{ date: string }>(
      `SELECT date FROM manual_debt_entries
    ...
    await reconcileDebtSnapshots(transaction, id);
  });
}
...
async function getDebtBalanceAtDate(
  db: SQLite.SQLiteDatabase,
  id: number,
  throughDate: string,
  excludePaymentId = -1
  ...
  const row = await db.getFirstAsync<{ balance: number }>(
    MANUAL_DEBT_BALANCE_AT_DATE_SQL,
    ...manualDebtBalanceAtDateParams(id, throughDate, excludePaymentId)
  );
...
async function getDebtPaymentCapacityAtDate(
  ...
  const debt = await db.getFirstAsync<{
    ...
    'SELECT date(created_at) AS creation_date, balance_updated_at FROM manual_debts WHERE id = ?',
    debtId
  );
  if (!debt) throw new Error(t('database.debtMissing'));
  if (paymentDate < debt.creation_date) {
    throw new Error(t('database.debtPaymentBeforeCreation'));
  }
...
async function reconcileDebtSnapshots(db: SQLite.SQLiteDatabase, debtId: number): Promise<void> {
  ...
  for (const snapshot of snapshots) {
    ...
    const difference = Number(snapshot.reported_balance) - runningBalance;
    if (difference !== Number(snapshot.amount)) {
      await db.runAsync('UPDATE manual_debt_entries SET amount = ? WHERE id = ?', difference, snapshot.id);
    }
    runningBalance = Number(snapshot.reported_balance);
    boundaryDate = snapshot.date;
...
export async function addDebtBalanceAdjustment(debtId: number, data: NewDebtBalance): Promise<void> {
  ...
  if (!isValidIsoDate(data.date) || data.date > toLocalIsoDate(new Date())) {
    throw new Error(t('database.debtAdjustmentDateInvalid'));
  }
  const db = await getDb();
  await withExclusiveTransaction(db, async (transaction) => {
    const debt = await transaction.getFirstAsync<{ type: string; balance_updated_at: string | null }>(
      'SELECT type, balance_updated_at FROM manual_debts WHERE id = ?', debtId
    );
    if (!debt) throw new Error(t('database.debtMissing'));
...
export async function deleteDebtBalanceAdjustment(debtId: number, entryId: number): Promise<void> {
  const db = await getDb();
  await withExclusiveTransaction(db, async (transaction) => {
    const entry = await transaction.getFirstAsync<{ id: number }>(
      `SELECT id FROM manual_debt_entries
       WHERE id = ? AND debt_id = ? AND kind = 'adjustment' AND reported_balance IS NOT NULL`,
      entryId, debtId
    );
    if (!entry) throw new Error(t('database.debtAdjustmentMissing'));
    await transaction.runAsync('DELETE FROM manual_debt_entries WHERE id = ?', entryId);
    await reconcileDebtSnapshots(transaction, debtId);
    await transaction.runAsync('UPDATE manual_debts SET updated_at = CURRENT_TIMESTAMP WHERE id = ?', debtId);
  });
...
export async function updateExpense(
  ...
  await withExclusiveTransaction(db, async (transaction) => {
    ...
    if (debtEntry) {
      ...
      await reconcileDebtSnapshots(transaction, debtEntry.debt_id);
      await transaction.runAsync(
    ...
    const recurringResult = await transaction.runAsync(
      `INSERT INTO recurring_incomes
        (name, amount, frequency, interval_months, execution_day, registration_mode, start_date,
         end_date, next_date, active, source_income_id, payment_method_id, category_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?)`,
      ...
      incomeResult.lastInsertRowId, data.paymentMethodId, data.categoryId ?? null
    );
    await transaction.runAsync(
...
export async function createRecurringIncomeFromSource(
  ...
  await withExclusiveTransaction(db, async (transaction) => {
    const source = await transaction.getFirstAsync<{
      ...
      'SELECT id, name, amount, date, recurring_income_id, payment_method_id, category_id FROM incomes WHERE id = ?', sourceIncomeId
    );
    ...
    const result = await transaction.runAsync(
      ...
      source.payment_method_id, source.category_id
    );
...
export async function updateRecurringIncome(id: number, data: NewRecurringIncome): Promise<void> {
  ...
  const result = await db.runAsync(
    `UPDATE recurring_incomes SET name = ?, amount = ?, frequency = ?, interval_months = ?,
      ...
      payment_method_id = ?, category_id = ?, updated_at = CURRENT_TIMESTAMP
     WHERE id = ?`,
    ...
    data.paymentMethodId, data.categoryId ?? null, id
  );
        ...
        if (existing?.status !== 'generated' && existing?.status !== 'skipped') {
          if (rule.registration_mode === 'confirmation') {
            ...
            const income = await transaction.runAsync(
              'INSERT INTO incomes (name, amount, period_id, date, recurring_income_id, payment_method_id, category_id) VALUES (?, ?, ?, ?, ?, ?, ?)',
              rule.name, rule.amount, period.id, nextDate, rule.id, rule.payment_method_id, rule.category_id
            );
            await transaction.runAsync(
              `INSERT INTO recurring_income_occurrences
                (recurring_income_id, scheduled_date, status, income_id)
...
export async function approveRecurringIncomeOccurrence(
  ...
  await withExclusiveTransaction(db, async (transaction) => {
    const rule = await transaction.getFirstAsync<{ name: string; amount: number; payment_method_id: number | null; category_id: number | null }>(
      'SELECT name, amount, payment_method_id, category_id FROM recurring_incomes WHERE id = ?', recurringIncomeId
    );
    ...
    const income = await transaction.runAsync(
      'INSERT INTO incomes (name, amount, period_id, date, recurring_income_id, payment_method_id, category_id) VALUES (?, ?, ?, ?, ?, ?, ?)',
      rule.name, rule.amount, period.id, scheduledDate, recurringIncomeId, rule.payment_method_id, rule.category_id
    );
